...
public class UpdateServiceRequestValidator : AbstractValidator<UpdateServiceRequest>
{
    public UpdateServiceRequestValidator()
    {
        ...
        RuleFor(x => x.PromotionalPrice)
            .GreaterThan(0).When(x => x.PromotionalPrice.HasValue)
            .WithMessage("Promotional price must be greater than 0")
            .LessThan(x => x.Price).When(x => x.PromotionalPrice.HasValue)
            .WithMessage("Promotional price must be less than regular price");
    }
} 